        ...
        if ($this->getRequest()->getParam('isAjax')) {
            ...
            if (!count($postItems)) {
                ...
                foreach (array_keys($postItems) as $modelid) {
                    /** @var \Mycompany\Library\Model\Books $model */
                    $model = $this->_objectManager->create('Mycompany\Library\Model\Books')->load($modelid);
                    try {
                        $model->setData(array_merge($model->getData(), $postItems[$modelid]));
                        $model->save();
                    } catch (\Exception $e) {
                        $messages[] = "[Books ID: {$modelid}]  {$e->getMessage()}";
                        $error = true;
                    }
                }
            }
